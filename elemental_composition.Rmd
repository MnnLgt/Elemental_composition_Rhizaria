---
title: "Elemental composition and silica production of siliceous Rhizaria"
author: "Manon Laget"
date: "21/04/2022"
...
counts_ref <- data %>% group_by(reference, area, group) %>%
...
Getting minimum and maximum values for C and N content for this study:
...
### Volume to nitrogen content relationship
...
ggsave2("results/loglog_vol_carbon.png", reg_carbon)
ggsave2("results/loglog_vol_nitrogen.png", reg_nitrogen)
ggsave2("results/loglog_vol_elemental_legend.png", legend)
...
ggsave2("results/densities_carbon.png", p_densities)
ggsave2("results/densities_legend.png", legend)
...
ggsave2("results/densities_nitrogen.png", p_densities)
...
tab_cn <- data %>% 
  ...
  dplyr::select(umolN_per_ind, umolC_per_ind) %>%
  filter(!is.na(umolN_per_ind)) %>%
  filter(!is.na(umolN_per_ind))

cor.test(tab_cn$umolN_per_ind, tab_cn$umolC_per_ind,
    method = "kendall")
```
...
ggsave("results/boxplots_C_N.png", plot = bp_cn)
...
p_sic <- ggplot(z, aes(x = medC, y = medSi)) + 
  ...
  #geom_text(aes(label = round(medSi/medC, 2)),
  #              position = position_nudge(x = -0.08, y = 0.1)) +
  scale_x_log10(breaks = 10^seq(-1,5), minor_breaks = NULL) +
  scale_y_log10(breaks = 10^seq(-1,5), minor_breaks = NULL) +
  theme_bw() +
  theme(text = element_text(size = 14), element_line(size = 0.4),
        aspect.ratio = 10/12) +
  scale_color_carto_d(name = "Group", palette = "Safe") +
  xlab(TeX("Carbon content (µmolC cell$^{-1}$)")) +
  ylab(TeX("Biogenic silica content (µmolSi cell$^{-1}$)"))

p_sic

ggsave("results/ratios_sic.png", p_sic)
...
supp_si32 <-data_si32 %>%
  dplyr::select(group, type, esd_mean, nmolSi_percell_perday,
         nmolSi_percell, Vp) %>%
  group_by(group, type) %>%
  dplyr::summarise(mean_esd = as.integer(esd_mean),
                   sd_esd = as.integer(esd_mean),
...
data_nutrients <- readxl::read_xlsx("data/P2107_Nutrients.xlsx") %>%
  ...
  dplyr::select(Date_GMT, Silicic_Acid_umolperL)
